...
\begin{itemize}
    ...
    \item Los componentes auxiliares del microcontrolador son componentes eléctricos que aseguran el correcto funcionamiento del dsPIC. En el caso específico de este microcontrolador, es recomendable incluir varios condensadores en sus pines de alimentación.
\end{itemize}
...
\begin{itemize}
    ...
    \item Puerto entrada salida: para recibir señales digitales y analógicas, las cuales son procesadas por el microcontrolador. En este proyecto, estos puertos se utilizan para monitorizar los finales de carrera de la estructura del brazo robótico.
    
    \item Puerto PWM: capaz de generar señales PWM, es decir, señales digitales con un ancho de pulso variable, las cuales son necesarias para controlar los servomotores.
...
Teniendo en cuenta la información expuesta anteriormente, se ha decidido conectar en serie ambos reguladores, consiguiendo de esta forma una regulación de $9V$ a $5V$ y posteriormente una regulación de $5V$ a $3.3V$. El diagrama esquemático final es el siguiente\footnote{No se realiza un análisis más detallado de "bypass" y desacoplo, porque no se presuponen escenarios comprometedores ni en la alimentación del sistema ni en las cargas del mismo}\textsuperscript{,}\footnote{Todos los reguladores trabajan lejos de sus márgenes de potencia máxima por lo que no es necesario instalar disipadores térmicos para evacuar calor} (ver imagen \ref{fig:Esquematico_Alimentación}):
...
\begin{itemize}
    ...
    \item Puerto PWM, para el control de servomotores controlando así la posición angular de los mismos.
    
    Los generadores de señal PWM de este microcontrolador poseen las siguientes características técnicas relevantes:
    \begin{itemize}
        \item El microcontrolador ofrece 6 generadores de señal PWM, cada uno de los cuales puede generar dos señales PWM.
        \item Cada uno de los generadores posee un registro de 16 bits para la selección de la duración del ciclo de trabajo.
    \end{itemize}
    ...
    \end{figure}
     ...
     Mediante la información obtenida del \textit{pinout} del microcontrolador, se tiene que los generadores de PWM 1, 4, 2 y 3 tienen asignados los pines 15, 13, 11 y 9, respectivamente.
    ...
    Se ha tomado la decisión de incluir dos puertos UART independientes en la \ac{PCB} que se ha desarrollado, con el objetivo de dedicar uno de los canales a envío y recepción de instrucciones, mientras que el otro se usa para realizar labores de depuración y pruebas.
    ...
    Su conexionado es realizado utilizando los pines reconfigurables 41, 42 y 43, los cuales pueden ser usados para habilitar una salida digital. Se evalúa un nivel alto para encender el LED mientras que un nivel bajo para apagarlo.
...
El proceso de enrutado se ha realizado utilizando ambas capas de la \ac{PCB}, esto quiere decir que se han trazado pistas en la capa de soldadura y en la capa de componentes.
...
\begin{itemize}
    \item Intensidad de corriente que conducirá la pista. Distinción entre pistas de alimentación y de comunicación de señales digitales.
...
\begin{itemize}
    ...
    \caption{Tratado de las resinas mediante insolado \cite{universidaddegranadaConstruccionPCB}.}
    \label{fig:Insolado}
    \end{figure}
    
    Tal y como se puede ver en la imagen anterior (imagen \ref{fig:Insolado}), las superficies de la resina que han sido insoladas (amarillo claro), se convierten en reactivas al revelador, ya que en esa zona de la fotomáscara existe una transparencia. En las zonas en las cuales la transparencia es opaca e impide el paso de la luz ultravioleta la resina no se ve afectada y se mantiene no reactiva con el revelador.
    
    \begin{figure}[H]
    \centering 
    \includegraphics[width=0.8\linewidth]{pictures/Proceso3.JPG}
    \caption{Resultado tras el insolado \cite{universidaddegranadaConstruccionPCB}.}
    \end{figure}
    ...
    \caption{Proceso de revelado \cite{universidaddegranadaConstruccionPCB}.}
    \label{fig:Eliminado_Revelado}
    \end{figure}
    
    Tras someter la \ac{PCB} al proceso de revelado, se obtiene el siguiente resultado (imagen \ref{fig:Resultado_Revelado}):
    
    \begin{figure}[H]
    \centering 
    \includegraphics[width=0.7\linewidth]{pictures/Proceso5.JPG}
    \caption{Resultado tras revelado \cite{universidaddegranadaConstruccionPCB}.}
    ...
    \caption{Resultado tras atacado \cite{universidaddegranadaConstruccionPCB}.}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Icono de trazado de placas]{\includegraphics[width=60mm]{pictures/ImprimirPCB.PNG}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Máscara frontal.]{\includegraphics[width=80mm]{pictures/MascaraFrontal.PNG}}
   ...
   El taladrado de todos los orificios de la \ac{PCB}, es decir, \textit{pads} de componentes, vías y mecanizado de sujeción, se debe realizar con un taladro que cuente con una broca del tamaño adecuado, además de un soporte que aporte la estabilidad suficiente:
    ...
    \subfigure[Taladrado de los distintos orificios para componentes.]{\includegraphics[width=80mm]{pictures/Taladrado2.jpg}}
   ...
   Tras el proceso de guillotinado y taladrado se obtiene el siguiente resultado:
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Preparado de la placa en el horno.]{\includegraphics[width=.48\linewidth]{pictures/HORNO.jpg}}
    \hfill
    \subfigure[Precalentado del horno a 260 \textdegree C.]{\includegraphics[width=.48\linewidth]{pictures/HORNO2.jpg}}
    \subfigure[Proceso de endurecimiento (1) - se deja reposar la pasta para que se solidifique.]{\includegraphics[width=.48\linewidth]{pictures/HORNO4.jpg}}
    \hfill
    \subfigure[Proceso de endurecimiento (2) - se aplican golpes de calor para fijar la pasta.]{\includegraphics[width=.48\linewidth]{pictures/HORNO3.jpg}}
   ...
   Este proceso debe respetar un perfil de calentamiento en varias etapas que persiguen la actuación de las resinas de difusión y posteriormente la realización de las soldaduras propiamente dichas. Los límites de temperatura y los tiempos de cada etapa deben establecerse siguiendo las recomendaciones de los fabricantes de los componentes y de la pasta de soldadura.
    ...
    \begin{figure}[H]
    \centering
    \subfigure{\includegraphics[width=.49\linewidth]{pictures/Soldando.jpg}}
    \hfill
    \subfigure{\includegraphics[width=.49\linewidth]{pictures/Soldando2.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Cara de componentes.]{\includegraphics[width=.49\linewidth]{pictures/SoldaduraDelanteras.jpg}}
    \hfill
    \subfigure[Cara de soldadura.]{\includegraphics[width=.49\linewidth]{pictures/SoldadurasTraseras.jpg}}
    \caption{Integrantes del equipo soldando componentes.}
    \end{figure}
...
Tras completar la fabricación de la \ac{PCB}, es recomendable realizar algunas comprobaciones mínimas para asegurarse de que el prototipo construido opera:
...
\begin{itemize}
   ...
   Dado que el fallo no parecía deberse al proceso de fabricación, se procedió a revisar de nuevo los diseños lógico y físico de la \ac{PCB}. Tras esta revisión, se concluyó que el conexionado del diagrama lógico era correcto pero la huella física elegida para el regulador de tensión LM317 en el diagrama físico presentaba un orden en los pines diferente al del utilizado en el prototipo. Se muestran las imágenes en las que se ve claramente esta diferencia:
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Pines conectados erróneamente.]{\includegraphics[width=70.5mm]{pictures/PREFIX.png}}
    \subfigure[Pines conectados según el encapsulado utilizado.]{\includegraphics[width=69mm]{pictures/POSFIX.png}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Vista detallada del puente.]{\includegraphics[width=.39\linewidth]{pictures/EmpalmeDetallado.jpg}}
    \hfill
    \subfigure[Vista de los cuatro puentes realizados]{\includegraphics[width=.59\linewidth]{pictures/EmpalmesTraseros.jpg}}
