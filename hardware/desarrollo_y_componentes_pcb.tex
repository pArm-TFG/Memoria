...
\begin{itemize}
    ...
    En la siguiente imagen \textbf{INSERTE REFERENCIA AQUÍ} se puede apreciar la fractura en la pista de cobre:
   ...
   Esto además fue comprobado y corroborado por el tutor, por lo que se optó por utilizar un hilo de grapinar para reconectar el puerto PWM inservible a otro de los pines del microcontrolador, el cual usaba un módulo PWM que funcionaba correctamente y que no estaba siendo usado. Se obtuvo el siguiente resultado:
    ...
    \label{fig:CAMBIAR!!!!!!!!!!}
    \end{figure}
    Tal y como se puede observar en las figuras anteriores \textbf{INSERTE REFERENCIA AQUÍ}, el reducido tamaño de la soldadura requirió que este proceso fuese realizado con máxima precisión. Además, por precaución, se eliminó parte de la pista original para evitar errores en la señal.
   ...
   Dado que el fallo no parecía deberse al proceso de fabricación, se procedió a revisar de nuevo los diseños lógico y físico de la \ac{PCB}. Tras esta revisión, se concluyó que el conexionado del diagrama lógico era correcto pero la huella física elegida para el regulador de tensión LM317 en el diagrama físico presentaba un orden en los pines diferente al del utilizado en el prototipo. Se muestran las imágenes en las que se ve claramente esta diferencia:
    ...
    Tal y como se puede observar en las imágenes anteriores \textbf{INSERTE REFERENCIA AQUÍ}:
    
    \begin{itemize}
    
        \item El esquemático lógico del LM317 consta de 3 pines, al igual que su encapsulado y huella física (ver figura \textbf{INSERTE REFERENCIA AQUÍ}).
        
        \item El encapsulado del LM317 tiene como referencia TO-220, con lo cual se debe seleccionar dicha huella física (ver figura \textbf{INSERTE REFERENCIA AQUÍ}).
        ...
        \item Los \textit{pads} 2 y 3 de la huella física, correspondientes a los pines \texttt{ADJUST} y \texttt{OUTPUT} del encapsulado del LM317, están intercambiados y en consecuencia, la colocación de los pines es errónea (ver figura \textbf{INSERTE REFERENCIA AQUÍ}).
    ...
    \subfigure[Pines arreglados\cite{MotorCorrienteContinua2020}.]{\includegraphics[width=69mm]{pictures/POSFIX.png}}
    \caption{Modificación del circuito de alimentación de los servomotores marcada en azul.}
    \label{fig:CAMBIAR!!!!!!!!!!}
    \end{figure}
    
    Esta modificación del diagrama físico de la \ac{PCB} hecha a posteriori ya se contempla en el apartado \textbf{CREAR UN LABEL A ESE PUNTO Y REFERENCIARLO AQUÍ} (5.4.5) de la memoria, dado que en dicho apartado se muestra la versión final del diagrama físico.
    ...
    Puesto que el fallo consistía en dos pines que deberían estar intercambiados, bastaría con intercambiar las pistas de comunicación que conectaban con ellos. Se decidió, de esta forma, fracturar las pistas existentes para después colocar dos puentes, intercambiando de esta forma el conexionado de los pines con sus pistas (ver figura \textbf{INSERTE REFERENCIA}).
    ...
    \subfigure[Vista de los cuatro puentes realizados\cite{MotorCorrienteContinua2020}]{\includegraphics[width=92.5mm]{pictures/EmpalmesTraseros.jpg}}
