...
\begin{figure}[H]
    ...
    \caption{Diagrama de conexionado \cite{noauthor_lm317_nodate}}
...
Se obtiene entonces que la ecuación funcional (ver ecuación \ref{eq:funcional}) a utilizar en el cálculo de $R_1$ y $R_2$ es:
...
Aplicando el conexionado recomendado por el fabricante y los cálculos para el valor de las resistencias se obtiene finalmente el diagrama esquemático del circuito de alimentación de los servomotores (ver imagen \ref{fig:Esquematico_Alimentacion_Servos}):
...
\begin{itemize}
    ...
    En el diagrama anterior (ver imagen \ref{fig:AZ1117H_Conexionado}) se utiliza el modelo que ofrece regulación de voltaje a $5V$. Sin embargo, el modelo usado en el proyecto es el que ofrece regulación de voltaje a $3.3V$, siendo su conexionado exactamente igual.
    
\end{itemize}

Teniendo en cuenta la información expuesta anteriormente, se ha decidido conectar en serie ambos reguladores, consiguiendo de esta forma una regulación de $9V$ a $5V$ y posteriormente una regulación de $5V$ a $3.3V$. El diagrama esquemático final es el siguiente (ver imagen \ref{fig:Esquematico_Alimentación}):
...
\begin{itemize}
    ...
    \item Puerto de programación mediante sonda: El conector que recibe la sonda debe tener una estructura específica y se describe en el \textit{datasheet} de la misma (ver imagen \ref{fig:Sonda_Conexionado}):
    ...
    \end{figure}
     ...
     Cabe destacar que el pin 18 o MCLR debe tener un conexionado específico en el que se emplea una resistencia \textit{pull-up}; esta estructura de conexión se muestra en el \textit{datasheet} del microcontrolador (ver imagen \ref{fig:MCL_Conexionado}):
     
    \begin{figure}[H]
    \centering 
    \includegraphics[width=.6\linewidth]{pictures/MCLR.PNG}
    \caption{Conexión del pin MCLR \cite{DsPIC33EP512GM60416BitMicrocontrollers}.}
    ...
    El diagrama esquemático que implementa esta funcionalidad es el siguiente (ver imagen \ref{fig:Tris_esquematico}):
    ...
    El esquema mostrado por el fabricante para los generadores PWM es el siguiente (ver imagen \ref{fig:PWM_Datasheet}):
    ...
    El conexionado final del puerto PWM en el diagrama esquemático es el siguiente (ver imagen \ref{fig:PWM_Esquematico}):
    ...
    El conexionado de los puertos UART  se realiza mediante pines reconfigurables del microcontrolador, en este caso se han utilizado los pines 2 y 3 para el primer canal; además de los pines 3 y 4 para el segundo canal. El diagrama esquemático final es el siguiente (ver imagen \ref{fig:Uart_Esquematico}):
    ...
    A continuación se muestra el diagrama esquemático del circuito (ver imagen \ref{fig:Led_Esquematico}):
...
En la imagen anterior ((ver imagen \ref{fig:Asignar_Huella_Ventana}) se muestra la ventana de asignación de huellas, la cual está dividida en tres secciones:
\begin{itemize}
...
Como ejemplo, en la imagen anterior (ver imagen \ref{fig:Asignar_Huella_Ventana}) se ha realizado una búsqueda dentro de la librería \textit{``capacitor \ac{THT}''}, ya que se quieren utilizar condensadores de agujero pasante en el diseño físico y por lo tanto, en la sección derecha de la ventana se muestran las huellas susceptibles de ser usadas, filtradas por número de pines y librería.

Una vez se ha elegido la huella que se quiere utilizar en el diseño físico, esta se asigna al componente esquemático y puede ser visualizada (ver imagen \ref{fig:Huella_Condensador}):
...
Mediante la herramienta anterior (ver imagen \ref{fig:Pcb_Calculator}) se puede realizar el cálculo para los dos tipos de pistas usadas en esta \ac{PCB}: pistas de alimentación y pistas de comunicación. Para el cálculo de ambos tipos, se asume un aumento de temperatura máximo de 10 ºC, una longitud del conductor de $20cm$ y un valor de resistividad de $1.72 \cdot 10^{-8}$.

En primer lugar, se considera que las pistas de alimentación de la \ac{PCB} conducen $2A$ de intensidad y un voltaje de $9V$ máximo. Teniendo en cuenta dichos datos, el ancho de pista obtenido es el siguiente (ver imagen \ref{fig:Ancho_Alimentacion}):
...
\end{figure}
 ...
 En segundo lugar, se considera que las pistas de comunicación del microcontrolador, conducen 0.25A y 3.3V máximo. Teniendo en cuenta dichos datos, el ancho de pista obtenido es el siguiente (ver imagen \ref{fig:Ancho_Comunicacion}):
...
Asumiendo un ancho mínimo de pista de $0.4mm$, las pistas de comunicación están sobredimensionadas por motivos justificados y por lo tanto se obtiene el siguiente cálculo (ver imagen \ref{fig:Calculo_Inverso}) :
...
\begin{figure}[htbp]
    \centering
    \subfigure[Capa frontal]{\includegraphics[width=.49\linewidth]{pictures/PlacaFinal3D.PNG}}
    \hfill
    \subfigure[Capa trasera]{\includegraphics[width=.49\linewidth]{pictures/PlacaFinal3D2.PNG}}
...
\begin{figure}[htbp]
\centering
\subfigure[Icono de verificación de reglas de diseño]{\includegraphics[width=82mm]{pictures/DRC2.PNG}}
\subfigure[Desplegable de la herramienta]{\includegraphics[width=80mm]{pictures/DRC.PNG}}
...
\begin{itemize}
    ...
    Tal y como se puede ver en la imagen anterior (ver imagen \ref{fig:Fotolitografia}), la placa de prototipado de fibra de vidrio posee tres capas: fibra de vidrio como material base encima de la cual hay una capa de cobre y, protegiendo la anterior, una capa de resina fotosensible a la luz ultravioleta.
    ...
    \caption{Tratado de las resinas mediante insolado \cite{noauthor_construccion_nodate}}
    \label{fig:Insolado}
    \end{figure}
    
    Tal y como se puede ver en la imagen anterior (ver imagen \ref{fig:Insolado}), las superficies de la resina que han sido insoladas (amarillo claro), se convierten en reactivas al revelador, ya que en esa zona de la fotomáscara existe una transparencia. En las zonas en las cuales la transparencia es opaca e impide el paso de la luz ultravioleta la resina no se ve afectada y se mantiene no reactiva con el revelador.
    
    \begin{figure}[H]
    \centering 
    \includegraphics[width=0.8\linewidth]{pictures/Proceso3.JPG}
    \caption{Resultado tras el insolado \cite{noauthor_construccion_nodate}}
    \end{figure}
    
    \item Mediante el proceso de revelado, se elimina la resina que fue tratada en el proceso de insolación (ver imagen \ref{fig:Eliminado_Revelado}):
    
    \begin{figure}[H]
    \centering 
    \includegraphics[width=0.7\linewidth]{pictures/Proceso4.JPG}
    \caption{Proceso de revelado \cite{noauthor_construccion_nodate}}
    \label{fig:Eliminado_Revelado}
    \end{figure}
    
    Tras someter la \ac{PCB} al proceso de revelado, se obtiene el siguiente resultado (ver imagen \ref{fig:Resultado_Revelado}):
    
    \begin{figure}[H]
    \centering 
    \includegraphics[width=0.7\linewidth]{pictures/Proceso5.JPG}
    \caption{Resultado tras revelado \cite{noauthor_construccion_nodate}}
    \label{fig:Resultado_Revelado}
    \end{figure}
    
    Tal y como se puede ver en la imagen anterior (ver imagen \ref{fig:Resultado_Revelado}), el proceso de revelado elimina la resina que había sido insolada, ya que se trata de una placa positiva. En estas zonas, la capa de cobre queda expuesta y sin ninguna protección. Por el contrario, en las zonas que no fueron insoladas, el revelado no actúa y por lo tanto la resina inicial permanece protegiendo la capa de cobre.
    ...
    \caption{Resultado tras atacado \cite{noauthor_construccion_nodate}}
    \label{fig:Resultado_Atacado}
    \end{figure}
    
    Tal y como se puede observar en la imagen anterior (ver imagen \ref{fig:Resultado_Atacado}), la capa de cobre solo permanece en los lugares que estaban protegidos por la resina inicial; este patrón coincide con el circuito de la fotomáscara. Para retirar la resina sobrante, bastan con aclarar la \ac{PCB} con alcohol, quedando las pistas del circuito perfectamente delimitadas e impresas en la placa de fibra de vidrio.
...
\begin{enumerate}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Icono de impresión de placas]{\includegraphics[width=80mm]{pictures/ImprimirPCB.PNG}}
    \subfigure[Desplegable de la herramienta]{\includegraphics[width=80mm]{pictures/ImprimirPCB2.PNG}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Máscara frontal]{\includegraphics[width=82mm]{pictures/MascaraFrontal.PNG}}
    \subfigure[Máscara trasera]{\includegraphics[width=80mm]{pictures/MascaraTrasera.PNG}}
   ...
   Para imprimir dichas máscaras en papel de transparencia, primeramente se imprimen en papel convencional y posteriormente se fotocopian a un papel de transparencia, obteniendo el resultado siguiente (ver imagen \ref{fig:Resultado_Recorte}):
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Máscaras de papel y sus transparencias]{\includegraphics[width=90mm]{pictures/MascarasTransparenciaPapel.jpg}}
    \subfigure[Recorte de las máscaras]{\includegraphics[width=75mm]{pictures/OperarioCortando.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Funda de transparencias ]{\includegraphics[width=60mm]{pictures/MáscaraTransparencia.jpg}}
    \subfigure[Demostración de su capacidad de contener a la placa]{\includegraphics[width=90mm]{pictures/Funda.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Placa de prototipado colocada dentro de la fotomáscara]{\includegraphics[width=75mm]{pictures/Isoladora.jpg}}
    \subfigure[Ajuste de la máquina de insolación]{\includegraphics[width=75mm]{pictures/Isoladora2.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Comienzo del proceso de revelado]{\includegraphics[width=75mm]{pictures/Revelado.jpg}}
    \subfigure[Proceso de revelado en estado avanzado]{\includegraphics[width=75mm]{pictures/Revelado2.jpg}}
    \subfigure[Aclarado de la \ac{PCB}]{\includegraphics[width=75mm]{pictures/Revelado3.jpg}}
   ...
   A continuación, se muestran algunas imágenes (ver imágenes \ref{fig:Atacado_PCB}) del proceso de atacado mediante ácido:
   

   \begin{figure}[H]
    \centering
    \subfigure[Recipiente contenedor de la disolución de atacado]{\includegraphics[width=75mm]{pictures/Atacado.jpg}}
    \subfigure[\ac{PCB} siendo sometida al proceso de atacado]{\includegraphics[width=75mm]{pictures/Atacado2.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Parte frontal de la \ac{PCB}]{\includegraphics[width=75mm]{pictures/Impreso.jpg}}
    \subfigure[Parte trasera de la \ac{PCB}]{\includegraphics[width=85mm]{pictures/Impreso2.jpg}}
    ...
    \caption{Integrante del equipo verificando cortos.}
    \end{figure}
   ...
   El taladrado de todos los orificios de la \ac{PCB}, es decir, pads de componentes, vías y mecanizado de sujeción; se debe realizar con un taladro que cuente con una broca del tamaño adecuado, además de un soporte que aporte la estabilidad suficiente:
    ...
    \begin{figure}[H]
    \centering
    \subfigure[ Taladro y soporte usado]{\includegraphics[width=80mm]{pictures/Taladrado.jpg}}
    \subfigure[Taladrado de los]{\includegraphics[width=80mm]{pictures/Taladrado2.jpg}}
   ...
   Tras el proceso de guillotinado y taladrado se obtiene el siguiente resultado:
    ...
    Tal y como se puede ver en la imagen (ver imagen \ref{fig:Micro_Detalle}), el dsPIC utilizado tiene un encapsulado destinado a soldadura superficial (\ac{SMD}). Normalmente, este tipo de componentes deben ser colocados y soldados los primeros, ya que el proceso de colocación y soldado de los mismos es delicado y requiere máxima precisión.
    ...
    \begin{figure}[H]
    \centering
    \subfigure[M]{\includegraphics[width=60mm]{pictures/HORNO.jpg}}
    \subfigure[C]{\includegraphics[width=60mm]{pictures/HORNO2.jpg}}
    \subfigure[]{\includegraphics[width=60mm]{pictures/HORNO4.jpg}}
    \subfigure[]{\includegraphics[width=60mm]{pictures/HORNO3.jpg}}
    \caption{Diferentes etapas del proceso de horneado.}
    \end{figure}
    
    Este proceso se debe realizar durante unos quince minutos a una temperatura de entre 250 \textdegree C y 260 \textdegree C.
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Vía con hilo de grapinar sin cortar]{\includegraphics[width=80mm]{pictures/Via.jpg}}
    \subfigure[Aspecto final de la vía.]{\includegraphics[width=72.5mm]{pictures/VIA2.jpg}}
    \caption{Taladrado de la \ac{PCB}.}
    \end{figure}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[]{\includegraphics[width=70mm]{pictures/Soldando.jpg}}
    \subfigure[]{\includegraphics[width=72.5mm]{pictures/Soldando2.jpg}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Cara de componentes]{\includegraphics[width=70mm]{pictures/SoldaduraDelanteras.jpg}}
    \subfigure[Cara de soldadura]{\includegraphics[width=70mm]{pictures/SoldadurasTraseras.jpg}}
    \caption{Integrantes del equipo soldando componentes.}
    \end{figure}
...
\begin{itemize}
    ...
    Este contratiempo afecta al periférico PWM por partida doble, ya que se detectó una fractura en una de las pistas de comunicación del puerto PWM y, a su vez, se comprobó que el módulo PWM interno que usaba esta pista también estaba defectuoso y era inservible.
    
    En la siguiente imagen (ver imagen \ref{fig:Fractura}) se puede apreciar la fractura en la pista de cobre:
   ...
   Esto además fue comprobado y corroborado por el tutor, por lo que se optó por utilizar un hilo de grapinar para reconectar el puerto PWM inservible a otro de los pines del microcontrolador, el cual usaba un módulo PWM que funcionaba correctamente y que no estaba siendo usado. Se obtuvo el siguiente resultado:
    ...
    \end{figure}
    Tal y como se puede observar en las figuras anteriores (ver imágenes \ref{fig:Reconex} y \ref{fig:Reconex_Soldadura}), el reducido tamaño de la soldadura requirió que este proceso fuese realizado con máxima precisión. Además, por precaución, se eliminó parte de la pista original para evitar errores en la señal.
   ...
   Dado que el fallo no parecía deberse al proceso de fabricación, se procedió a revisar de nuevo los diseños lógico y físico de la \ac{PCB}. Tras esta revisión, se concluyó que el conexionado del diagrama lógico era correcto pero la huella física elegida para el regulador de tensión LM317 en el diagrama físico presentaba un orden en los pines diferente al del utilizado en el prototipo. Se muestran las imágenes en las que se ve claramente esta diferencia:
    ...
    Tal y como se puede observar en las imágenes anteriores (ver imágenes \ref{fig:Logico_LM317}, \ref{fig:Huella_LM317} y \ref{fig:Encapsulado_LM317}):
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Pines erróneos]{\includegraphics[width=70.5mm]{pictures/PREFIX.png}}
    \subfigure[Pines arreglados]{\includegraphics[width=69mm]{pictures/POSFIX.png}}
    ...
    \begin{figure}[H]
    \centering
    \subfigure[Vista detallada del puente]{\includegraphics[width=70mm]{pictures/EmpalmeDetallado.jpg}}
    \subfigure[Vista de los cuatro puentes realizados]{\includegraphics[width=92.5mm]{pictures/EmpalmesTraseros.jpg}}
    \caption{Puentes entre pistas de los reguladores de tensión LM317.}
    \label{fig:puentes}
    \end{figure}
    
    Para la realización se emplearon cables de tamaño reducido que actuaban como puente entre las pistas. Debido dicho tamaño y dado que el soldado de los mismos se realizó a mano, se considera que el resultado podría ser mejorable; sin embargo, esta solución dió buenos resultados y corrigió correctamente el voltaje de salida de los reguladores de tensión LM317.
