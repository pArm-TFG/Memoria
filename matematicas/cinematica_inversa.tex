...
está situado sobre el plano $XZ$, por lo que de los puntos anteriores se sabe que el 
\textit{end--effector} se sitúa en: $P_{ee} = \left(x', z'\right)$, por lo que se puede
definir un vector desde la base $\overrightarrow{x'z'}$ que represente el lado `$b$'.
Como se puede apreciar, no se usan los puntos $\left(x, z\right)$ originales sino que
previamente es necesario modificarlos:
\begin{itemize}
    ...
    \end{equation}
    \item Además, para la coordenada anterior, se define una dependencia respecto a la
    posición en `$y$', ya que dicha posición define (según la vista cenital) el largo del
    brazo y la posición del mismo. Por ello, no se puede físicamente estirar el brazo sin
    alterar la distancia en `$x$' (ver figura \ref{fig:y_movement}):
    \begin{figure}[H]
        \centering
        \includegraphics[width=.5\linewidth]{pictures/y_coord_movevment.png}
        \caption{Demostración de cómo un ajuste en la posición `$y$' provoca una alteración en `$x$'.}
        \label{fig:y_movement}
    \end{figure}

    De esta manera, la coordenada $x'$ se define como:
    \begin{equation*}
        x' = \frac{x'}{\sin{\theta_0}}
    \end{equation*}
...
\begin{equation*}
    b = \left|\overrightarrow{x'z'}\right| = \sqrt{x'^2 + z'^2}
\end{equation*}
...
\begin{itemize}
    ...
    y cuya longitud es: $b = \left|\overrightarrow{x'z'}\right| = \sqrt{x'^2 + z'^2}$.
\end{itemize}
...
\begin{align}
    \theta_1' & = \arccos{\left(\frac{-\overline{A_L}^2 - \left(x'^2 + z'^2\right) + \overline{A_U}^2} %
    {-2 \cdot \overline{A_L} \cdot \sqrt{x'^2 + z'^2}}\right)} \label{eq:t1} \\[2ex]
    \theta_2' & = \arccos{\left(\frac{-\overline{A_L}^2 - \overline{A_U}^2 + x'^2 + z'^2} %
    {-2 \cdot \overline{A_L} \cdot \overline{A_U}}\right)} \label{eq:t2}
\end{align}
...
$\theta_1$ como el ángulo $\theta_2$ que aparecen en el triángulo de la figura \ref{fig:ik_triangle}
