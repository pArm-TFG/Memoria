...
class Ui(QtWidgets.QMainWindow):
    ...
    def check_list(self, theta_0, theta_1, theta_2, x_coord, y_coord, z_coord):
        '''
        This method is used to verify whether a given arm's position is reachable
        or not.
        ...
        if theta_0 > 151:
            result = False
            if self.counter == 200:
                self.log.warning('Base joint angle (t0) is over 151')
                self.counter = 0
            self.counter += 1 
        ...
        if theta_1 > 135:
            result = False
            if self.counter == 200:
                self.log.warning('Shoulder joint angle (t1) is over 135')
                self.counter = 0
            self.counter += 1 
        ...
        if theta_2 > 120:
            result = False
            if self.counter == 200:
                self.log.warning('Elbow joint angle (t2) is over 120')
                self.counter = 0
            self.counter += 1 
